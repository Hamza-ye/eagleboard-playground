...
@Service( "com.mass3d.preheat.PreheatService" )
...
//        preheat.setDefaults( manager.getDefaults() );
        ...
        for ( Class<? extends IdentifiableObject> klass : params.getObjects().keySet() )
        {
            params.getObjects().get( klass ).stream()
                ...
                .forEach( identifiableObject -> ((BaseIdentifiableObject) identifiableObject).setUid( CodeGenerator
                    .generateUid() ) );
        }
        ...
        else if ( PreheatMode.REFERENCE == params.getPreheatMode() )
        {
            ...
            if ( uidMap != null && (
                PreheatIdentifier.UID == params.getPreheatIdentifier() || PreheatIdentifier.AUTO == params.getPreheatIdentifier()) )
            {
            ...
            if ( codeMap != null && (
                PreheatIdentifier.CODE == params.getPreheatIdentifier() || PreheatIdentifier.AUTO == params.getPreheatIdentifier()) )
            {
...
//                }
        ...
        IdentifiableObject refObject, IdentifiableObject ref )
    {
        ...
        if ( refObject == null && DataElement.class.isInstance( object ) )
        {
            return null;
        }
