...
public interface SecurityService {
  ...
  /**
   * Indicates whether a restore/invite is allowed for the given user. The requirements are:
   * <p>
   * <ul>
  ...
  /**
   * Indicates whether an invite is allowed for the given user. Delegates to validateRestore(
   * UserCredentials ). The requirements are.
   * <p>
   * <ul>
   * <li>no_user_credentials</li>
   ...
   * user.
   * <p>
   * In the case of inviting a user to finish setting up an account, the user account must already
   * be configured with the profile desired for the user (e.g., locale, organisation unit(s),
   * role(s), etc.)
   *
   ...
   * @return false if any of the arguments are null or if the user credentials identified by the
   * user name does not exist, true otherwise.
   */
  boolean sendRestoreMessage(UserCredentials credentials, String rootPath,
      RestoreOptions restoreOptions);

  /**
   * Populates the restoreToken property of the given credentials with a hashed version of
   ...
   * @param credentials the user credentials.
   * @param restoreOptions restore options, including type of restore.
   * @return an array where index 0 is the clear-text token and index 1 the clear-text code.
   */
  ...
  /**
   * Tests whether the given token is valid for the given user name. If true, it will update the
   * user credentials identified by the given user name with the new password. In order to succeed,
   * the given token must match the ones on the credentials, and the current date must be before the
   ...
   */
  boolean restore(UserCredentials credentials, String token, String newPassword,
      RestoreType restoreType);

  /**
   * Tests whether the given token and code are valid for the given user name. In order to succeed,
   * the given token and code must match the ones on the credentials, and the current date must be
   * before the expiry date time of the credentials.
   *
   * @param credentials the user credentials.
   * @param token the token.
   * @param code the code.
   ...
   * the hashed version of the token matches the one on the user credentials identified by the given
   * user name.
   *
   * @param credentials the user credentials.
   * @param token the token.
   * @return error message if any of the arguments are null or if the user credentials identified by
   * the user name does not exist, null if the arguments are valid.
   */
  String verifyToken(UserCredentials credentials, String token, RestoreType restoreType);

  /**
   * Indicates whether the given username is an invite. The username is considered an invite if it
   * is null or matches the invite username pattern of invite-<email>-<uid>.
   *
