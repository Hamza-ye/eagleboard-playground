...
import com.mass3d.hibernate.exception.ReadAccessDeniedException;
...
public class HibernateIdentifiableObjectStore<T extends BaseIdentifiableObject>
  ...
  public List<T> getAll() {
    ...
    return getList(builder,
        new JpaQueryParameters<T>().addPredicates(getSharingPredicates(builder)));
  }
  ...
  public List<T> getAllLikeName(String name, boolean caseSensitive) {
    ...
    if (caseSensitive) {
      ...
      likePredicate = root -> builder
          .like(builder.lower(root.get("name")), "%" + name.toLowerCase() + "%");
    }
  ...
  public List<T> getAllLikeName(Set<String> nameWords, int first, int max) {
    ...
    param.addPredicate(root -> builder.and(
        conjunction.stream().map(p -> p.apply(root)).collect(Collectors.toList())
  ...
  /**
   * Creates a detached criteria with data sharing restrictions relative to the given user and
   * access string.
   *
   * @param user the user.
  ...
  /**
   * Creates a detached criteria with sharing restrictions relative to the given user and access
   * string.
   *
   * @param user the user.
   * @param access the access string.
  ...
  /**
   * Creates a sharing Criteria for the implementation Class type restricted by the given
   * Criterions.
   *
