...
import com.mass3d.interpretation.InterpretationService;
import com.mass3d.interpretation.InterpretationStore;
import com.mass3d.interpretation.MentionUtils;
import com.mass3d.interpretation.NotificationType;
...
@Service( "com.mass3d.interpretation.InterpretationService" )
@Transactional
...
//        sendMentionNotifications( interpretation, null, users );
  ...
  private int sendNotificationMessage(Set<User> users, Interpretation interpretation,
    ...
    String fullBody = String.join("\n\n", Arrays.asList(
        ...
        String
            .format("%s %s", i18n.getString("go_to") /*, getInterpretationLink( interpretation )*/)
    ));
  ...
  private void notifySubscribers(Interpretation interpretation, InterpretationComment comment,
    ...
    Schema interpretableObjectSchema = schemaService
        .getDynamicSchema(interpretableObject.getClass());
  ...
  public void updateSharingForMentions(Interpretation interpretation, Set<User> users) {
    for (User user : users) {
      if (!aclService.canRead(user, interpretation.getObject())) {
        interpretation.getObject().getUserAccesses()
            .add(new UserAccess(user, AccessStringHelper.READ));
      }
    }
  }
