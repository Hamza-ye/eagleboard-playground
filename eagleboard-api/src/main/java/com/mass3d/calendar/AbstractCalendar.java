...
public abstract class AbstractCalendar implements Calendar {
  ...
  @Override
  public List<DateInterval> toIntervals(DateTimeUnit dateTimeUnit, DateIntervalType type,
      int offset, int length, int periods) {
  ...
  public boolean isValid(DateTimeUnit dateTime) {
    ...
    return dateTime.getDay() >= 1 && dateTime.getDay() <= daysInMonth(dateTime.getYear(),
        dateTime.getMonth());
  }
}
