...
import com.mass3d.calendar.DateUnitPeriodTypeParser;
import com.mass3d.calendar.DateUnitType;
import com.mass3d.calendar.PeriodTypeParser;
...
public abstract class PeriodType
  ...
  private static com.github.benmanes.caffeine.cache.Cache<String, Period> PERIOD_CACHE = Caffeine.newBuilder()
  ...
  public static PeriodType getByNameIgnoreCase(String name) {
    for (PeriodType periodType : getAvailablePeriodTypes()) {
      if (name != null && periodType.getName().toLowerCase().trim()
   ...
   */
  public static List<Period> getPeriodTypePeriods(Period period,
  ...
  private String getCacheKey(com.mass3d.calendar.Calendar calendar, Date date) {
  ...
  public Period createPeriod(Calendar cal) {
    com.mass3d.calendar.Calendar calendar = getCalendar();
   ...
   * @return the valid Period based on the given date
   */
  public Period createPeriod(final Date date, final com.mass3d.calendar.Calendar calendar) {
  ...
  protected Period toIsoPeriod(DateTimeUnit start, DateTimeUnit end,
      com.mass3d.calendar.Calendar calendar) {
  ...
  public abstract Period createPeriod(DateTimeUnit dateTimeUnit,
      com.mass3d.calendar.Calendar calendar);
   ...
   * @return a Period which is the next of the given Period.
   */
  public Period getNextPeriod(Period period, com.mass3d.calendar.Calendar calendar) {
    DateTimeUnit dateWithOffset = getDateWithOffset(
        createLocalDateUnitInstance(period.getStartDate(), calendar),
        1, calendar);
   ...
   * @return a Period which is the previous of the given Period.
   */
  public Period getPreviousPeriod(Period period, com.mass3d.calendar.Calendar calendar) {
    DateTimeUnit dateWithOffset = getDateWithOffset(
        createLocalDateUnitInstance(period.getStartDate(), calendar),
        -1, calendar);
   ...
   * @param period how many periods to go back(if negative) or forward(if positive). A value of 0
