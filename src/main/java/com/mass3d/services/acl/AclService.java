package com.mass3d.services.acl;

import com.mass3d.api.common.IdentifiableObject;
import com.mass3d.api.feedback.ErrorReport;
...
public interface AclService {
   ...
   * <p/>
   * 1. Does user have ACL_OVERRIDE_AUTHORITIES authority? 2. Is the user for the object null? 3. Is
   * the user of the object equal to current user? 4. Is the object public read? 5. Does any of the
   * userGroupAccesses contain public read and the current user is in that group
   *
   ...
   *
   * Data-read permission is only considered if the given object's schema is 'DataShareable'. If not
   * 'DataShareable', only metadata-read ACL is considered
   *
   * @param user User to check against
   ...
   * <p/>
   * 1. Does user have ACL_OVERRIDE_AUTHORITIES authority? 2. Is the user for the object null? 3. Is
   * the user of the object equal to current user? 4. Is the object public write? 5. Does any of the
   * userGroupAccesses contain public write and the current user is in that group
   *
   * @param user User to check against
   * @param object Object to check
   * @return Result of test
   ...
   * <p/>
   * 1. Does user have ACL_OVERRIDE_AUTHORITIES authority? 2. Does user have the authority to create
   * public instances of that object
   *
   * @param user User to check against
   * @param klass Class to check
   * @return Result of test
   ...
   * <p/>
   * 1. Does user have ACL_OVERRIDE_AUTHORITIES authority? 2. Does user have the authority to create
   * private instances of that object
   *
   * @param user User to check against
   * @param klass Class to check
   * @return Result of test
