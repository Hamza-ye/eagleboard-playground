...
import com.mass3d.support.hibernate.HibernateMetadata;
...
//        .put( AnalyticalObject.class, BaseAnalyticalObject.class )
  ...
  protected void updateJoinTables() {
    ...
    Iterator<?> collectionIterator = metamodelImplementor.collectionPersisters().values()
        .iterator();
    ...
    while (collectionIterator.hasNext()) {
      ...
      if (collectionPersister.isManyToMany() && collectionType.isAssociationType()) {
        Joinable associatedJoinable = collectionType
    ...
    Iterator<Map.Entry<String, List<String>>> entryIterator = joinTableToRoles.entrySet()
        .iterator();
  ...
  protected Map<String, Property> getPropertiesFromHibernate(Class<?> klass) {
    ...
    try {
      ...
      // class is not persisted with configuration
      return new HashMap<>();
    }
    ...
    while (propertyIterator.hasNext()) {
      ...
      if (property.isCollection()) {
        ...
        CollectionPersister persister = metamodelImplementor
