...
import com.mass3d.api.common.NameableObject;
import com.mass3d.api.node.annotation.NodeAnnotation;
import com.mass3d.api.node.annotation.NodeCollection;
import com.mass3d.api.node.annotation.NodeComplex;
import com.mass3d.api.node.annotation.NodeRoot;
import com.mass3d.api.node.annotation.NodeSimple;
...
public class NodePropertyIntrospectorService extends AbstractPropertyIntrospectorService {
  ...
  private void handleNodeCollection(NodeCollection nodeCollection, Property property) {
    ...
    {
      if (property.getItemKlass() != null && property.getItemKlass()
          .isAnnotationPresent(NodeRoot.class)) {
  ...
  private Method getMethodWithPrefix(Class<?> klass, Field field, List<String> prefixes,
      boolean includeType) {
    ...
    if (methods.size() > 1) {
      log.error(
          "More than one method found for field " + field.getName() + " on class " + klass.getName()
