...
public class Property implements Ordered, Klass {
   ...
   * assumed to be a primitive type. If collection is true, this this check is done on the generic
  ...
  /**
   * Does this class implement EmbeddedObject ?
   */
  ...
  /**
   * The configuration role of the owning side.
   */
  private String owningRole;

  /**
   * The configuration role of the inverse side (if many-to-many).
   */
  ...
  public int hashCode() {
    return Objects
        .hash(klass, propertyType, itemKlass, itemPropertyType, getterMethod, setterMethod, name,
            fieldName, persisted, collectionName,
            collectionWrapping, description, namespace, attribute, simple, collection, owner,
            identifiableObject, nameableObject, readable, writable,
            unique, required, length, max, min, cascade, manyToMany, oneToOne, manyToOne,
            owningRole, inverseRole, constants);
  }
