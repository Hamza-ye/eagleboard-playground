...
public class HibernateGenericStore<T>
  ...
  public HibernateGenericStore(SessionFactory sessionFactory, JdbcTemplate jdbcTemplate,
      Class<T> clazz, boolean cacheable) {
  ...
  /**
   * Creates a Query for given HQL query string. Must specify the return type of the Query
   * variable.
   *
   * @param hql the HQL query.
   ...
   */
  public TypedQuery<T> getExecutableTypedQuery(CriteriaQuery<T> criteriaQuery) {
    return getSession()
  ...
  protected void preProcessPredicates(CriteriaBuilder builder,
      List<Function<Root<T>, Predicate>> predicates) {
  }
   ...
   * @return executable TypedQuery
   */
  protected TypedQuery<T> getTypedQuery(CriteriaBuilder builder,
    ...
    if (!orderProviders.isEmpty()) {
      List<Order> orders = orderProviders.stream().map(o -> o.apply(root))
          .collect(Collectors.toList());
...
//  }
  ...
  public int getCount() {
    ...
    return getCount(builder,
        newJpaParameters().count(root -> builder.countDistinct(root.get("id")))).intValue();
  }
