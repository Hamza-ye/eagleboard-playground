...
public class MathUtils {
  ...
  private static final Pattern POSITIVE_OR_ZERO_INT_PATTERN = Pattern
      .compile("(^0$)|(^[1-9]\\d*$)");
   ...
   *
   * @param expression The expression to validate.
   * @return The error information for an invalid expression, null if the expression is valid.
   */
  ...
  public static Object getRoundedObject(Object value) {
    return value != null && Double.class.equals(value.getClass()) ? getRounded((Double) value)
        : value;
  }
  ...
  /**
   * Rounds a number to a given number of significant decimal digits. Note that the number will be
   * left with *only* this number of significant digits regardless of magnitude, e.g. 12345 to 3
   * digits will be 12300, whereas 0.12345 will be 0.123.
   *
   * @param value the value to round off.
  ...
  /**
   * Rounds the fractional part of a number to a given number of significant decimal digits. Digits
   * to the left of the decimal point will not be rounded. For example, rounding 12345 to 3 digits
   * will be 12345, whereas 12.345 will be 12.3, and 0.12345 will be 0.123.
   *
   * @param value the value to round off.
  ...
  /**
   * Returns the given value if between the min and max value. If lower than minimum, returns
   * minimum, if higher than maximum, returns maximum.
   *
  ...
  /**
   * Returns true if the provided string argument is to be considered numeric. Matches using a
   * lenient pattern where leading zeros are allowed.
   *
   * @param value the value.
   * @return true if the provided string argument is to be considered numeric.
   */
  public static boolean isNumericLenient(String value) {
    return value != null && DOUBLE_VALIDATOR.isValid(value, LOCALE) && NUMERIC_LENIENT_PATTERN
        .matcher(value).matches();
  }

  /**
   * Returns true if the provided string argument is to be considered a unit interval, which implies
  ...
  public static boolean isPositiveInteger(String value) {
    return value != null && INT_VALIDATOR.isValid(value) && POSITIVE_INT_PATTERN.matcher(value)
        .matches();
  }
  ...
  public static boolean isZeroOrPositiveInteger(String value) {
    return value != null && INT_VALIDATOR.isValid(value) && POSITIVE_OR_ZERO_INT_PATTERN
        .matcher(value).matches();
  }
  ...
  public static boolean isNegativeInteger(String value) {
    return value != null && INT_VALIDATOR.isValid(value) && NEGATIVE_INT_PATTERN.matcher(value)
        .matches();
  }
  ...
  /**
   * Indicates if the provided string argument is to be considered as a boolean, more specifically
  ...
  /**
   * Tests whether the two decimal numbers are equal with a tolerance of 0.01. If one or both of the
   * numbers are null, false is returned.
   *
  ...
  /**
   * Parses the given string and returns a double value. Returns null if the given string is null or
  ...
  /**
   * Parses an integer silently. Returns the Integer value of the given string. Returns null if the
   * input string is null, empty or if it cannot be parsed.
   *
  ...
  /**
   * Returns the lower bound for the given standard deviation, number of standard deviations and
   * average.
   *
  ...
  /**
   * Returns the high bound for the given standard deviation, number of standard deviations and
   * average.
   *
   * @param stdDev the standard deviation.
