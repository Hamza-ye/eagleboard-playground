...
public class ValidationUtils {
  ...
  public static final ImmutableSet<String> ILLEGAL_SQL_KEYWORDS = ImmutableSet
      .of("alter", "before", "case",
          "commit", "copy", "create", "createdb", "createrole", "createuser", "close", "delete",
          "destroy", "drop",
          "escape", "insert", "select", "rename", "replace", "restore", "return", "update", "when",
          "write");
  ...
  private static final Pattern TIME_OF_DAY_PATTERN = Pattern
      .compile("^([0-9]|0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]$");
  private static final Pattern BBOX_PATTERN = Pattern
      .compile("^" + NUM_PAT + ",\\s*?" + NUM_PAT + ",\\s*?" + NUM_PAT + ",\\s*?" + NUM_PAT + "$");
  private static final Pattern INTERNATIONAL_PHONE_PATTERN = Pattern
      .compile("^\\+(?:[0-9].?){4,14}[0-9]$");
  ...
  public static boolean expressionIsValidSQl(String filter) {
    ...
    for (int i = 0; i < filter.length(); i++) {
      ...
      if (!(Character.isWhitespace(ch) || Character.isLetterOrDigit(ch) || SQL_VALID_CHARS
          .contains(ch))) {
        return false;
      }
    }
  ...
  public static boolean passwordIsValid(String password) {
    ...
    return DIGIT_PATTERN.matcher(password).matches() && UPPERCASE_PATTERN.matcher(password)
        .matches();
  }
  ...
  public static boolean coordinateIsValid(String coordinate) {
    ...
    return longitude >= LONG_MIN && longitude <= LONG_MAX && latitude >= LAT_MIN
        && latitude <= LAT_MAX;
  }
  ...
  /**
   * Returns the longitude from the given coordinate. Returns null if the coordinate string is not
   * valid. The coordinate is on the form longitude / latitude.
   *
  ...
  /**
   * Returns the latitude from the given coordinate. Returns null if the coordinate string is not
   * valid. The coordinate is on the form longitude / latitude.
   *
   * @param coordinate the coordinate string.
   * @return the latitude.
  ...
  /**
   * Returns a coordinate string based on the given latitude and longitude. The coordinate is on the
...
//    }
  ...
  public static String dataValueIsValid(String value, ValueType valueType) {
    ...
    } else if (ValueType.INTEGER_ZERO_OR_POSITIVE == valueType && !MathUtils
        .isZeroOrPositiveInteger(value)) {
...
//        }
  ...
  /**
   * Indicates whether the given value is zero and not zero significant according to its data
   * element.
   *
   * @param value the data value.
   * @param dataElement the data element.
  ...
  public static boolean dataValueIsZeroAndInsignificant(String value, DataElement dataElement) {
    ...
    return dataElement.getValueType().isNumeric() && MathUtils.isZero(value) && !dataElement
        .isZeroIsSignificant() &&
  ...
  /**
   * Checks if the given comment is valid. Returns null if valid and a string if invalid, possible
   * values are:
   * </p>
   * <ul>
  ...
  /**
   * Checks if the given stored by value is valid. Returns null if valid and a string if invalid,
   * possible values are:
   * </p>
   * <ul>
